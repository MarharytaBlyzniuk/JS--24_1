...
class SkillsManager {
  constructor() {
    this.skills = [];// Ініціалізуємо порожній масив для зберігання навичок
  }

addSkill(skill) {
  if (typeof skill === 'string' && skill.length >= 2) {          // Перевіряємо, чи аргумент є рядком і чи має мінімум два символи
    this.skills.push(skill); // Додаємо навичку до масиву
    return skill;// Повертаємо додану навичку
  }
  return null;  // Якщо умови не виконані, повертаємо null
}
getAllSkills() {
  return this.skills; // Повертаємо масив усіх навичок
}
}
 ...
 * Завдання: Створити модуль на JavaScript, який імплементує функцію-конструктор DateCalculator для створення об'єктів, здатних
 *  керувати датами. Калькулятор дат має надавати такі можливості:
...
function DateCalculator(initialDate) {
  ...
  this.getResult = function() {
   const year = this.date.getFullYear();
    const month = String(this.date.getMonth() + 1).padStart(2, '0'); // Ensure two digits
    const day = String(this.date.getDate()).padStart(2, '0'); // Ensure two digits
   return `${year}-${month}-${day}`;
  }
}

const dateCalculator = new DateCalculator('2024-08-01');
